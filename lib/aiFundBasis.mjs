...
export function fundRowMetrics(wrapper, price, { anthVal, oaiVal, dilution, resolved }) {
  ...
  const anthRound = wrapper.anthropic?.roundVal;
  const oaiRound = wrapper.openai?.roundVal;
  const anthPct = anthRound ? fundClaimPct(resolved.anthFv, anthRound) : claimPct(wrapper.anthropic);
  const oaiPct = oaiRound ? fundClaimPct(resolved.oaiFv, oaiRound) : claimPct(wrapper.openai);
