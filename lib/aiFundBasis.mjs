...
export function fundRowMetrics(wrapper, price, { anthVal, oaiVal, dilution, resolved }) {
  ...
  const liveTnaProxy =
    denomKind === "netAssets" ? ARKVX_CLASS_D_SO * price : null;
  const wrapperValue =
    denomKind === "netAssets"
      ? (resolved.netAssets ?? liveTnaProxy)
      : mcap;
  const nav =
    resolved.nav ??
    (wrapper.ticker === "AGIX" ? price : null);
  const netAssets =
    denomKind === "netAssets"
      ? wrapperValue
      : resolved.netAssets ?? (nav != null ? nav * shares : null);
  const premium =
    nav > 0 && price > 0 && wrapper.type === "Fund" && denomKind !== "netAssets"
      ? price / nav - 1
      : null;
  ...
  if (resolved.deployRange && denomKind === "netAssets" && liveTnaProxy > 0) {
    const bookVal = lookThroughPer100({
      claimPct: anthPct,
      ipoVal: anthVal,
      marketCap: ARKVX_NPORT_TNA,
      dilution,
    });
    const bookOai = lookThroughPer100({
      claimPct: oaiPct,
      ipoVal: oaiVal,
      marketCap: ARKVX_NPORT_TNA,
      dilution,
    });
    anthPer100High = bookVal;
    oaiPer100High = bookOai;
  } else if (resolved.deployRange && resolved.anthFvHigh !== resolved.anthFv) {
