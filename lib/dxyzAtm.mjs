...
export function computeAtmBridge({
  ...
  const bridgeAprShares = aprMayShares ?? cal.aprMayShares;
  const bridgeAprPrice = aprMayAvgPrice ?? cal.aprMayAvgPrice;
  const bridgeParticipation = participation ?? cal.participation;
  ...
  const postMay = simulatePostMay({
    ...
    commissionRate,
    capacityGross,
    startingNetAssets: markedNetAssets + aprMayNet,
    startingShares: baselineShares + bridgeAprShares,
    minPremium,
  });
