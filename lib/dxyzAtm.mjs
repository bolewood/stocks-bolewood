...
// count: 57,591,678 − round($1B / $61.66) ≈ 41,373,060.
...
// todayNY must be a "YYYY-MM-DD" string in America/New_York.
export function completedTradingRows(rows, todayNY) {
  return rows.filter((r) => r.date < todayNY);
}
...
export function simulatePostMay({
  ...
  // Rates outside their meaningful ranges (typed past UI limits) would produce
  // nonsense like negative proceeds with positive dilution.
  commissionRate = Math.min(Math.max(commissionRate, 0), 1);
...
export function computeAtmBridge({
  ...
  let bridgeAprShares = aprMayShares ?? cal.aprMayShares;
  let bridgeAprPrice = aprMayAvgPrice ?? cal.aprMayAvgPrice;
  const bridgeParticipation = Math.max(participation ?? cal.participation, 0);
  ...
  const effAsOf = asOfDate ?? lastDate;
