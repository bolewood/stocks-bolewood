...
export function computeAtmBridge({
  ...
  // May 22–23: shelf exhausted, new program not yet effective — zero issuance.
  const priorRemaining = priorShelfRemainingApr1();
  let aprMayGross = bridgeAprShares * bridgeAprPrice;
  let aprMayCapped = false;
  if (aprMayGross > priorRemaining) {
    aprMayGross = priorRemaining;
    bridgeAprPrice = bridgeAprShares > 0 ? aprMayGross / bridgeAprShares : 0;
    aprMayCapped = true;
  }
