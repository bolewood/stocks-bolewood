...
//             OR a same-ET-day print older than 15m (OTC delay, interval NAV)
// CLOSE       cash session closed; quote is the last completed session's print
// STALE       cash session open and the print is from a prior ET day
...
export function classifyQuote(
  ...
  if (isCashSessionOpen(now)) {
    ...
    if (age >= PRICE_THRESHOLDS.liveMaxQuoteAgeMs) {
      const q = etClock(quoteAsOf);
      const n = etClock(now);
      const sameEtDay =
        q.year === n.year && q.month === n.month && q.day === n.day;
      // OTC/interval prints often lag >15m the same morning. STALE is for a
      // feed still sitting on a prior day's close while the tape is open.
      return sameEtDay ? "cached" : "stale";
    }
...
export function newestQuoteAsOf(quotes, tickers) {
  let newest = null;
  for (const ticker of tickers) {
    const t = quotes?.[ticker]?.quoteAsOf;
...
export function chipQuoteAsOf(state, quotes, tickers) {
  if (state === "stale" || state === "unavailable") {
    return oldestQuoteAsOf(quotes, tickers);
  }
  return newestQuoteAsOf(quotes, tickers);
}
